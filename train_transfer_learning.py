import torch
import models
from args import parse_transfer_learning_args
from utils import *
...
def loss_compute(args, model, criterion, outputs, targets):
    ...
    if args.sep_decay:
        ...
        loss += lamb * (torch.sum(w ** 2) + torch.sum(b ** 2)) + \
            lamb_feature * torch.sum(features ** 2)
...
def main():
    args = parse_transfer_learning_args()
    device = torch.device("cuda:"+str(args.gpu_id)
                          if torch.cuda.is_available() else "cpu")
    args.device = device
    train_loader, _, num_classes = make_dataset(
        args.dataset, args.data_dir, args.batch_size, args.sample_size, SOTA=args.SOTA)
    # Load the trained model
    model = models.__dict__[args.model](
        num_classes=num_classes, fc_bias=args.bias, ETF_fc=args.ETF_fc, fixdim=args.fixdim, SOTA=args.SOTA).to(device)
    # for i in range(args.epochs):
    #     model.load_state_dict(torch.load(
    #         args.load_path + 'epoch_' + str(i + 1).zfill(3) + '.pth'))
    #     freeze(model)
    if torch.cuda.is_available():
        pretrained_dict_old = torch.load(
            args.load_path + 'epoch_' + '200' + '.pth')
    else:
        pretrained_dict_old = torch.load(
            args.load_path + 'epoch_' + '200' + '.pth', map_location=torch.device('cpu'))
    model_dict = model.state_dict()
    pretrained_dict = {k: v for k,
                       v in pretrained_dict_old.items() if 'fc' not in k}
